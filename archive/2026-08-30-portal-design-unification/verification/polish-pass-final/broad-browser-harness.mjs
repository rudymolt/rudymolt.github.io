...
const OUT = HERE;
const SHOTS = join(OUT, "broad-screenshots");
const CHROME = process.env.CHROME_BIN || "google-chrome";
const HEAD = "c289088672c98fee30e6a370f1e166afa4a67cab";
const MAX_RETRIES = 2;
...
async function captureFullPage(page, debugPort, filename) {
  ...
  try {
    ...
    try {
      ...
      const composed = await composer.cdp.send("Page.captureScreenshot", { format: "jpeg", quality: 70, fromSurface: true, captureBeyondViewport: true, clip: { x:0, y:0, width:rasterWidth, height:rasterHeight, scale:1 } });
...
async function main() {
  ...
  const profile = await mkdtemp(join(tmpdir(), "portal-polish-final-chrome-"));
  const harnessErrors = [];
  ...
  try {
    ...
    await writeFile(join(OUT, "broad-browser-results.json"), `${JSON.stringify(results, null, 2)}\n`);
  }
